/* ══════════════════════════════════════════════════════════════
   雪夜 · 交互雪景 —— v10-PC（基于 v10，仅删除移动端适配）
   保留：v10 全部细节设定（楼/灯高度大小、路灯开关、滚雪球、
         堆雪人、双击碎裂、铲雪车归位/清场、脚印、积雪物理）
   删除：触屏事件(touchStarted/Moved/Ended)、长按检测、
         LONG_PRESS_MS、viewport/touch-action 等移动端代码
   防错：所有模板字符串改为 + 拼接，避免复制污染导致黑屏
══════════════════════════════════════════════════════════════ */
...
const CONFIG = {
  ...
  HUD_TEXT_SIZE: 12,          // HUD 字号 ｜ 建议 11–14
...
/* 颜色工具：用 + 拼接（避免反引号复制出错导致黑屏） */
const rgba = function(c, a) { return "rgba(" + c[0] + "," + c[1] + "," + c[2] + "," + (a / CONFIG.ALPHA_FULL).toFixed(3) + ")"; };
